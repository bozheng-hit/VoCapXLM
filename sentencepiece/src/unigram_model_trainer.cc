...
const TrainerModel::Vocabulary &TrainerModel::GetRequiredVocabulary() const {
  return required_vocab_;
}
...
void TrainerModel::LoadAndSetRequiredVocabulary(const std::string &vocab_path) {
  TrainerModel::Vocabulary required_vocab;
  std::string line;
  std::ifstream in(trainer_spec_.vocab_path());
  int i = 0;
  while (std::getline(in, line)) {
    const std::vector<std::string> v = absl::StrSplit(line, '\t');
    // LOG(INFO) << v[0] << required_vocab.size();
    // CHECK_EQ(v.size(),2);
    std::string w = v[0];
    if (i < 50) {
      LOG(INFO) << w << " " << required_vocab.size();
    }
    if (w == "<s>" || w == "</s>" or w == "<unk>") {
      LOG(INFO) << "omiting meta piece: " << w;
      continue;
    }
    required_vocab.insert(w);
    i += 1;
  }
  required_vocab_ = std::move(required_vocab);
  LOG(INFO) << required_vocab_.size() << " required pieces loaded.";
  CHECK(status().ok());
}
...
//   CHECK_OK(sp.Load(vocab_path));
//   LOG(INFO) << sp.GetPieceSize();
//   for (int i = 3; i < sp.GetPieceSize(); i ++) {
//     if (i < 50) {
//       LOG(INFO) << sp.IdToPiece(i) << " " << sp.GetScore(i);
//     }
//     required_vocab.insert(sp.IdToPiece(i));
//   }
//   required_vocab_ = std::move(required_vocab);
//   LOG(INFO) << required_vocab_.size() << " required pieces loaded.";
//   CHECK(status().ok());
...
// TrainerModel::SentencePieces Trainer::InitSentencePiecesFromModel() const {
//   sentencepiece::SentencePieceProcessor sp;
//   CHECK_OK(sp.Load(trainer_spec_.model_path()));
//   LOG(INFO) << sp.GetPieceSize();
...
//       LOG(INFO) << sp.IdToPiece(i) << " " << sp.GetScore(i);
//     }
//     seed_sentencepieces.emplace_back(sp.IdToPiece(i), sp.GetScore(i));
//   }
//   return seed_sentencepieces;
// }


TrainerModel::SentencePieces TrainerModel::MergeRequiredVocabulary(SentencePieces seed_sentencepieces) {
  float min_log_prob = 1e9;
  TrainerModel::Vocabulary existing_words;
  const auto &required_vocab = GetRequiredVocabulary();

  for (const auto &p : seed_sentencepieces) {
    min_log_prob = std::min(min_log_prob, p.second);
    if (port::ContainsKey(required_vocab, p.first)) {
      existing_words.insert(p.first);
    }
  }

  LOG(INFO) << existing_words.size() << " existing pieces from required vocabulary";

  int cre = 0;
  for (const auto &w: required_vocab) {
    if (!port::ContainsKey(existing_words, w)) {
      seed_sentencepieces.emplace_back(w, min_log_prob);
      cre ++;
    }
  }
  LOG(INFO) << "Merging " << cre << " seed pieces from required vocabulary";
  CHECK(status().ok());
  return seed_sentencepieces;
}
...
TrainerModel::SentencePieces Trainer::RunMStep(
  ...
  const auto &required_vocab = model.GetRequiredVocabulary();
  ...
  for (size_t i = 0; i < expected.size(); ++i) {
    ...
    if (freq < kExpectedFrequencyThreshold && !port::ContainsKey(required_vocab, sentencepieces[i].first)) {
      continue;
    } else if (freq < kExpectedFrequencyThreshold && port::ContainsKey(required_vocab, sentencepieces[i].first)) {
      new_sentencepieces.emplace_back(sentencepieces[i].first, kExpectedFrequencyThreshold);
      sum += kExpectedFrequencyThreshold;
    } else {
...
TrainerModel::SentencePieces Trainer::PruneSentencePieces(
  ...
  std::vector<bool> must_keep(sentencepieces.size(), false);
  ...
  for (size_t i = 0; i < sentencepieces.size(); ++i) {
    ...
    if(port::ContainsKey(required_vocab,w.first)){
      must_keep[i] = true;
      continue;
    }
...
TrainerModel::SentencePieces Trainer::FinalizeSentencePieces(
  ...
  for(const auto &w : required_vocab){
    ...
    if (port::ContainsKey(final_sentencepieces, w)) {
      continue;
    }
    if (port::ContainsKey(sp, w)) {
      final_sentencepieces[w] = sp[w];
    } else {
      // Add penalty to avoid required pieces from having the same score.
      // Since the required_chars_ is sorted, frequent pieces have
      // less penalties.
      final_sentencepieces[w] = model.min_score() + min_score_penalty;
      min_score_penalty += kMinScorePenaltyDelta;
      cre++;
    }
  }
  LOG(INFO) << "Merging " << cre << " pieces from required vocabulary";
...
util::Status Trainer::Train() {
  ...
  if (trainer_spec_.vocab_path().size() > 0) {
    model.LoadAndSetRequiredVocabulary(trainer_spec_.vocab_path());
  }

  if (trainer_spec_.train_extremely_large_corpus()) {
    TrainerModel::SentencePieces seed_sentencepieces = MakeSeedSentencePieces<int64>();
    seed_sentencepieces = model.MergeRequiredVocabulary(seed_sentencepieces);
    model.SetSentencePieces(std::move(seed_sentencepieces));
  } else {
    TrainerModel::SentencePieces seed_sentencepieces = MakeSeedSentencePieces<int32>();
    seed_sentencepieces = model.MergeRequiredVocabulary(seed_sentencepieces);
    model.SetSentencePieces(std::move(seed_sentencepieces));
  }
  ...
  LOG(INFO) << "final pieces " << final_pieces_.size();
  return Save();
}
}  // namespace unigram
}  // namespace sentencepiece
