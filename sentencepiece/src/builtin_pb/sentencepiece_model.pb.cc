...
class TrainerSpec::_Internal {
 public:
  ...
  static void set_has_vocab_path(HasBits* has_bits) {
    (*has_bits)[1] |= 10u;
  }
...
TrainerSpec::TrainerSpec(const TrainerSpec& from)
  ...
  if (from._internal_has_vocab_path()) {
    vocab_path_.Set(::PROTOBUF_NAMESPACE_ID::internal::ArenaStringPtr::EmptyDefault{}, from._internal_vocab_path(), 
      GetArena());
  }
...
// @@protoc_insertion_point(message_clear_start:sentencepiece.TrainerSpec)
  ...
  if (cached_has_bits & 0x00000010u) {
    vocab_path_.ClearNonDefaultToEmpty();
  }
  _has_bits_.Clear();
...
::PROTOBUF_NAMESPACE_ID::uint8* TrainerSpec::_InternalSerialize(
  ...
  if (cached_has_bits & 0x00000010u) {
    target = stream->WriteStringMaybeAliased(
        50, this->_internal_vocab_path(), target);
  }
