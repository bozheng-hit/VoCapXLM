...
class TrainerModel : public Model {
 public:
  ...
  using Vocabulary = std::set<std::string>;
  ...
  const Vocabulary &GetRequiredVocabulary() const;
  void LoadAndSetRequiredVocabulary(const std::string &vocab_path);
  SentencePieces MergeRequiredVocabulary(SentencePieces seed_sentencepieces);
